import type { AnyMiddleware, Middleware, AnyStandardSchema, InferOutput, Priority, Tags, ChannelDefinition } from '@emailrpc/core';
...
export type EmailSendArgs<TInput> = {
...
export class EmailBuilder<Ctx, S extends Slots = {}> {
  ...
  declare readonly _args: EmailSendArgs<InputForSlots<S>>;
  declare readonly _rendered: RenderedMessage;
  readonly _channel = 'email' as const;

  constructor(state: InternalBuilderState) {
    this._state = state;
  }

  _finalize(id: string): ChannelDefinition<EmailSendArgs<unknown>, RenderedMessage> {
    const state = this._state;
    if (!state.schema || !state.subject || !state.template) {
