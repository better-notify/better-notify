...
<Callout>
This guide assumes you have already installed the packages. If not, start with [Installation](./installation.mdx).
</Callout>
...
Same catalog, same routes, same client shape. Only the transport changed. Read more about the [mock transport](../transports/generic/mock.mdx).

## Where to go next

You now have a working setup with typed routes, sub-catalogs, and real email delivery. From here:

- Read [Channels](../concepts/channels.mdx) to understand how email, SMS, push, and custom channels work.
- Read [Catalog and Routes](../concepts/catalog-and-routes.mdx) for deeper coverage of route composition and dot-path IDs.
- Read [Transports Overview](../transports/index.mdx) to learn about multi-transport composition and failover.
- Read [Middleware](../concepts/middleware.mdx) when you want rate limits, tracing, or event logging across routes.
