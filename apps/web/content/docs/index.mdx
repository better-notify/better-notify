...
Better-Notify is typed notification infrastructure for Node.js. You define a notification catalog once, then use the generated client to send email, SMS, push, Telegram, or custom-channel messages through one consistent pipeline.
...
<Mermaid
  code={`
flowchart LR
  A[Channels<br/>email, sms, push, telegram, custom] --> B[createNotify]
  B --> C[rpc.catalog]
  C --> D[createClient]
