...
The Twilio transport is provided by `@betternotify/twilio`. It calls the [Twilio Messages API](https://www.twilio.com/docs/messaging/api/message-resource) via `fetch` with HTTP Basic Auth — no Twilio SDK required (Node 22+ / Bun built-in).
