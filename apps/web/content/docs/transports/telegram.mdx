...
The Telegram transport is included in `@betternotify/telegram`. It calls the [Telegram Bot API](https://core.telegram.org/bots/api) via `fetch` — no external HTTP dependencies required (Node 22+ / Bun built-in).
