...
The Resend transport is an official Better-Notify package that sends email through the [Resend](https://resend.com) HTTP API. It uses plain `fetch()` with zero external dependencies, so it works in Node.js, Bun, Cloudflare Workers, and any runtime with a global `fetch`.
