...
The Slack transport is included in `@betternotify/slack`. It calls the [Slack Web API](https://api.slack.com/methods/chat.postMessage) via `fetch` — no external HTTP dependencies required (Node 22+ / Bun built-in).
