...
Generate a key pair with `generateVapidKeys()`:
...
Both values are base64url-encoded strings. Store them as environment variables and reuse across deploys:
...
<Callout type="warn">
  Regenerating keys invalidates every existing browser subscription. Users would need to re-subscribe through your app.
</Callout>
