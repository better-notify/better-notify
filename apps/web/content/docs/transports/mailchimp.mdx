...
The Mailchimp Transactional transport sends email through the [Mandrill](https://mandrillapp.com) HTTP API (now part of Mailchimp). It uses plain `fetch()` with zero external dependencies, so it works in Node.js, Bun, Cloudflare Workers, and any runtime with a global `fetch`.
