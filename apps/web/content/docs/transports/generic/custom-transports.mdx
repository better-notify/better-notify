...
A transport is the wire-level adapter that delivers a fully-rendered message to a provider. Better-Notify ships transports for [SMTP](/docs/transports/third-party/smtp), [SES](/docs/transports/third-party/ses), and [Resend](/docs/transports/third-party/resend), but you can write your own for any provider that accepts an HTTP call, SDK method, or socket write.
...
Throwing also works. Both `{ ok: false, error }` and thrown errors are handled the same way by [multi-transport](/docs/transports/generic/multi-transport) strategies: the error is logged and the next transport is tried.
...
`verify()` runs during startup checks. `close()` runs during graceful shutdown. Both propagate through [multi-transport](/docs/transports/generic/multi-transport) composites — `verify` requires at least one inner transport to pass; `close` waits for all inner transports to finish.
...
const withTracking = mapTransport(sesTransport, (message, ctx) => ({
  ...message,
  headers: {
    ...message.headers,
