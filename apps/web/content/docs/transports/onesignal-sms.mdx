...
The OneSignal SMS transport is part of `@betternotify/onesignal`. It delivers SMS through the [OneSignal](https://onesignal.com) HTTP API. It uses plain `fetch()` with zero external dependencies, so it works in Node.js, Bun, Cloudflare Workers, and any runtime with a global `fetch`.
