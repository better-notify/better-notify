...
<TypeTable
  type={{
    name: {
      description: 'The template name as registered in the Meta Business Manager.',
      type: 'string | (args: { input, ctx }) => string',
    },
    language: {
      description: 'BCP-47 language code matching one of the registered template translations (e.g. `pt_BR`, `en_US`).',
      type: 'string | (args: { input, ctx }) => string',
    },
    components: {
      description: 'Optional array of header, body, and button components, each carrying its own `parameters` array. Matches Meta\'s wire shape verbatim — see the Meta transport docs for the full type. Omit for templates with no parameters.',
      type: 'WhatsAppTemplateComponent[] | (args: { input, ctx }) => WhatsAppTemplateComponent[]',
    },
  }}
/>
