...
- **Import from subpaths, not the root barrel, for runtime values.** `@betternotify/core` exports subpaths for optional features; always import runtime values from the specific subpath rather than the root. The root barrel (`@betternotify/core`) is reserved for core primitives only (`createCatalog`, `createClient`, `createNotify`, `defineChannel`, `slot`, `handlePromise`, `waitFor`, `obscureString`, `obscureEmail`, `consoleLogger`, `validate`, error classes, and type-only imports). Everything else must come from its subpath: `@betternotify/core/transports`, `@betternotify/core/middlewares`, `@betternotify/core/stores`, `@betternotify/core/sinks`, `@betternotify/core/tracers`, `@betternotify/core/logger`. This rule applies to all source files including examples — violating it re-introduces barrel bloat that undermines tree-shaking. Do not add runtime value re-exports back to `packages/core/src/index.ts`; type-only re-exports are fine.
