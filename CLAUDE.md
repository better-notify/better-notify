...
- **Import from subpaths, not the root barrel, for optional helpers.** Every `@betternotify/*` package that exposes multiple concerns (channel + transports, core + middlewares, etc.) must split them into subpath exports. The root barrel exports the primary API (channel definition, core primitives, types); everything else lives behind a subpath (`./transports`, `./middlewares`, `./stores`, etc.). Always import from the narrowest subpath — never pull transports or optional helpers from the root. This applies to all source files including examples. Violating it re-introduces barrel bloat that undermines tree-shaking. Type-only re-exports from the root are fine.
