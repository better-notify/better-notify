...
Adapter split rule (spec §3.2): **a feature ships as its own package only if it pulls a non-trivial peer dependency.** Otherwise it lives in `core` under a subpath export. Don't create new packages for thin wrappers. **Exception: provider-specific transports always get their own package**, regardless of peer dependency weight — they are substantive enough to warrant isolation even when using only `fetch()`.
...
- **100% test coverage is enforced.** The root `vitest.config.ts` sets thresholds: 100% lines, statements, functions, and branches. CI runs `pnpm test:coverage` in a dedicated `code-quality` job and fails if thresholds are not met. When adding new source files, write tests that cover every line and branch. Coverage excludes type-only files (`types.ts`, `*.types.ts`), barrel `index.ts` files, and test utilities — see the `coverage.exclude` array in `vitest.config.ts` for the full list.
...
- **Transport organization**: each transport implementation lives in `packages/core/src/transports/<name>.ts` (or in its own package like `@betternotify/smtp` for transports that pull non-trivial peers). Per-transport option types live in `<name>.types.ts` when applicable; the shared `Transport`/`TransportResult`/`TransportEntry` types live in `transports/types.ts`. Barrel re-exports through `transports/index.ts`. The `Transport` type is the wire-level email send adapter (SMTP, Resend) — distinct from `Sender` at Layer 2. Amazon SES users should use the SMTP transport with SES SMTP credentials.
- **Package naming by provider domain**: provider-specific adapter packages use domain-prefixed names: `cloudflare-email`, `aws-sqs`, etc. Generic protocol adapters (`smtp`) stay unprefixed. Single-domain providers where the provider name is already unambiguous (`resend`) stay as-is. This applies to all adapter packages, not just transports.
- **Inline exports only.** Always export at the declaration site (`export function`, `export type`, `export const`). Never use bottom-of-file re-export statements like `export { Foo }` or `export type { FooProps }`. Barrel `index.ts` files are the exception — re-exports are their purpose.
...
- **Import from subpaths, not the root barrel, for optional helpers.** `@betternotify/core` exports subpaths for optional features; always import those runtime values from the specific subpath rather than the root. The root barrel (`@betternotify/core`) exports core primitives (`createCatalog`, `isCatalog`, `createClient`, `createNotify`, `createPlugin`, `defineChannel`, `slot`, `handlePromise`, `waitFor`, `obscureString`, `obscureEmail`, `consoleLogger`, `validate`, error classes, and type-only imports) — these are fine to import from the root. Everything else must come from its subpath: `@betternotify/core/transports`, `@betternotify/core/middlewares`, `@betternotify/core/stores`, `@betternotify/core/sinks`, `@betternotify/core/tracers`, `@betternotify/core/logger`. This rule applies to all source files including examples — violating it re-introduces barrel bloat that undermines tree-shaking. Do not add runtime value re-exports for optional helpers back to `packages/core/src/index.ts`; type-only re-exports are fine.
- **Do not format CHANGELOG.md files.** `CHANGELOG.md` files are managed by release-please and should never be reformatted by oxfmt or included in commits that don't specifically update changelogs.
- **`apps/web` must never import `@betternotify/*` packages.** The web app (docs site / landing page) is a consumer-facing marketing site, not a library consumer. It must not depend on or import from `@betternotify/core` or any other `@betternotify/*` package. Use plain TypeScript utilities instead.
...
- **HTTP behavior is nested under `http?`** on all transport option types. `HttpClientBehaviorOptions` (retry, timeout, hooks) lives under an `http` key — not spread flat onto the transport options. This keeps transport-specific config (token, webhookUrl) visually separate from HTTP plumbing and avoids naming collisions between HTTP-level hooks (`onRequest`, `onError`) and client-level pipeline hooks (`onBeforeSend`, `onError`). Inside the `http` object, hook names are unprefixed (`onRequest`, not `onHttpRequest`) because the nesting provides the scope. The `createHttpClient` helper in `packages/core/src/transports/http.ts` composes client-level and per-request hooks via `mergeHooks`.
...
- **Per-send transport overrides via `TransportDataMap`**: `.send()` and `.batch()` accept an optional `transport` field — a provider-keyed object that flows through `SendContext` to the transport's `send(rendered, ctx)`. The `TransportDataMap` interface in `packages/core/src/transport.ts` uses declaration merging so provider packages register their key and body type. New transport packages should: (1) define a typed body type for their provider-specific fields, (2) augment `TransportDataMap` via `declare module '@betternotify/core'` in their barrel `index.ts`, (3) read `ctx.transport?.[key]` in their `send` method. Use `import('@betternotify/packagename').BodyType` in the `declare module` block to avoid rolldown type-name mangling. Multi-channel providers (e.g. OneSignal) register one key per channel (`onesignal-push`, `onesignal-email`, `onesignal-sms`) to avoid type leaking between channels. Single-channel providers use a flat key (e.g. `resend`, `twilio`). The merge order for request body fields is: `body` (global) → `bodyFor[route]` (per-route) → `ctx.transport[key]` (per-send) → mapped rendered fields (always win). `TransportDataMap` is one of the few places `interface` is used instead of `type` — declaration merging requires it.
