...
- **Package naming by provider domain**: provider-specific adapter packages use domain-prefixed names: `cloudflare-email`, `aws-ses`, `aws-sqs`, etc. Generic protocol adapters (`smtp`) stay unprefixed. Single-domain providers where the provider name is already unambiguous (`resend`) stay as-is. This applies to all adapter packages, not just transports.
